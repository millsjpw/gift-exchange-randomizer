...
def test_participant_exclusions():
    ...
    # Test participant with excluded and previous recipient
    p = Participant("Alice", excluded=["Bob"], previous_recipient="Charlie")

    assert not p.can_be_assigned("Alice")  # Cannot assign to self
    assert not p.can_be_assigned("Bob")   # Cannot assign to excluded
...
def test_spouse_constraints():
    """Test that excluded constraints are respected (formerly spouse)."""
    print("Testing excluded constraints...")

    participants = [
        Participant("Alice", excluded=["Bob"]),
        Participant("Bob", excluded=["Alice"]),
        Participant("Charlie"),
        Participant("Diana")
    ]
    ...
    print("✓ Excluded constraints working correctly")
...
def test_file_operations():
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
        test_data = {
            "participants": [
                {"name": "Alice", "excluded": ["Bob"], "previous_recipient": "Charlie"},
                {"name": "Bob", "excluded": ["Alice"], "previous_recipient": "Diana"},
                {"name": "Charlie", "excluded": [], "previous_recipient": "Alice"},
                {"name": "Diana", "excluded": [], "previous_recipient": "Bob"}
            ]
        }
